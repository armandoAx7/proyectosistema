...
$insertar= "INSERT INTO bajahdd (`folio`, `serie`, `modelo`, `fechabaja`, `observaciones`) VALUES ('$folio', '$serie', '$modelo', '$fechabaja', '$observaciones')";
//verificar que el usuario no se repita
$verificar_hdd =mysqli_query ($conexion,"SELECT * FROM bajahdd WHERE serie='$serie' ");

if(mysqli_num_rows($verificar_hdd)>0){
    
    header ("Location: bajahdd.php");
    
  echo '<script >
                    alert("el dispositivo ya se ha dado de baja");
                 window.location="bajahdd.php" 
                </script>'; 
